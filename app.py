...
from flask import (Flask, abort, request, session, flash,
...
def edit_recipe(recipe_id):
    ...
    if not recipe or recipe["user_id"] != session["user_id"] or recipe["status"] == 0:
        abort(403)
...
def search():
    ...
    results_count = rcps.search_count(query, tags) if query else 0
...
def edit_comment(comment_id):
    users_comment = cmmnt.get_comment(comment_id)

    if(not users_comment
       or users_comment["user_id"] != session["user_id"]
       or users_comment["status"] == 0):
        abort(403)

    if request.method == "GET":
        return render_template("edit_comment.html", comment=users_comment,
                               next_page="/recipe/" + str(users_comment["recipe_id"]))
    ...
    if request.method == "POST":
        ...
        cmmnt.update_comment(users_comment["id"], content)
        return redirect("/recipe/" + str(users_comment["recipe_id"]))
...
def delete_comment(comment_id):
    ...
    if request.method == "GET":
        return render_template("delete_comment.html", comment=users_comment)
